# utils.py
"""
Utility functions for Liquid Neural Network (LNN / LTC) workflows.

Contents
--------
1. ``export_to_json``              — Serialise a trained LNNModel to JSON.
...
5. ``plot_time_constants``         — Visualise the learned τ(x, h) landscape.
...
def export_to_json(model, filepath: str) -> None:
    """
    Serialise a trained ``LNNModel`` to a JSON file.

    The produced JSON contains three top-level keys:

    - ``recurrent_layers`` : Architecture metadata for every LTC cell,
                             including ``ode_unfolds``, ``dt``, and ``tau_min``.
    ...
    Args:
        model    : A trained ``LNNModel`` instance.
    ...
    for i, cell in enumerate(model.recurrent_cells):
        info = {
            ...
            'ode_unfolds': cell.ode_unfolds,
            'dt':          cell.dt,
            'tau_min':     cell.tau_min,
        }

        def _w(t): return t.detach().cpu().tolist()
        def _wb(m): return _w(m.bias) if m.bias is not None else None

        params[f'rec_{i}_W_ih_weight']  = _w(cell.W_ih.weight)
        params[f'rec_{i}_W_ih_bias']    = _wb(cell.W_ih)
        params[f'rec_{i}_W_hh_weight']  = _w(cell.W_hh.weight)
        params[f'rec_{i}_W_hh_bias']    = _wb(cell.W_hh)
        params[f'rec_{i}_W_tau_weight'] = _w(cell.W_tau.weight)
        params[f'rec_{i}_W_tau_bias']   = _wb(cell.W_tau)
        params[f'rec_{i}_W_tauh_weight']= _w(cell.W_tauh.weight)
        params[f'rec_{i}_A']            = _w(cell.A)
...
def plot_regression_uncertainty(
    ...
    """
    Plot predictive mean and uncertainty bands for a regression LNN.

    The function runs the model *n_samples* times (exploiting any stochasticity
    such as dropout) and displays the mean prediction with ±1 and ±2
    standard-deviation bands.

    Args:
        model     : Trained ``LNNModel`` instance.
    ...
    ax.fill_between(x_line,
                    ...
                    mean_preds + 2.0 * std_preds,
                    color='mediumorchid', alpha=0.25, label='±2 std')
    ax.fill_between(x_line,
                    mean_preds - std_preds,
                    mean_preds + std_preds,
                    color='mediumorchid', alpha=0.50, label='±1 std')
    ax.plot(x_line, mean_preds, color='darkviolet', linewidth=2,
...
def plot_decision_boundary(
    ...
    """
    Plot the decision boundary and epistemic uncertainty for a classification LNN.
    ...
    Args:
        ...
        n_samples : Number of forward passes (default: ``50``).
    ...
    grid_seq = grid.unsqueeze(1).expand(-1, seq_len, -1)
    ...
    if task == 'binary':
        ...
        contour = ax.contourf(xx, yy, Z_mean, levels=20, cmap='RdPu', alpha=0.8)
...
def plot_sequence_prediction(
    ...
    """
    Plot true versus predicted sequences for a sequence-to-sequence LNN.
    ...
    for i, ax in enumerate(axes):
        ...
        ax.plot(range(T), y_pred[i],    color='darkviolet',
...
# =============================================================================
# 5.  Time-constant landscape (LNN-specific)
# =============================================================================
...
def plot_time_constants(
    ...
    X_seq: torch.Tensor,
    layer_idx: int = 0,
    title: str = 'Liquid Time-Constant Landscape',
) -> None:
    """
    Visualise the learned time constants τ(x, h) across a sequence.
    """
    if layer_idx >= len(model.recurrent_cells):
        raise IndexError(
            f"layer_idx={layer_idx} is out of range "
            f"({len(model.recurrent_cells)} LTC layers available)."
        )

    cell = model.recurrent_cells[layer_idx]
    model.eval()

    taus = []

    with torch.no_grad():
        # 1. Propagate input through any preceding layers to get the correct features
        current_seq = X_seq
        for i in range(layer_idx):
            prev_cell = model.recurrent_cells[i]
            batch, seq_len, _ = current_seq.shape
            
            # Initialise hidden state for the preceding layer
            h_state = torch.zeros(batch, prev_cell.hidden_size, device=current_seq.device)
            out_seq = []
            
            # Process sequence step-by-step
            for t in range(seq_len):
                h_state = prev_cell(current_seq[:, t, :], h_state)
                out_seq.append(h_state.unsqueeze(1))
                
            current_seq = torch.cat(out_seq, dim=1)

        # 2. Now run the target layer with the correct input shape
        x = current_seq[0]           # (T, input_size_for_this_layer)
        h_prev = torch.zeros(1, cell.hidden_size, device=current_seq.device)

        for t in range(x.size(0)):
            x_t   = x[t].unsqueeze(0)                  # (1, input_size_for_this_layer)
            tau_t = cell._time_constant(x_t, h_prev)   # (1, hidden_size)
            taus.append(tau_t.squeeze(0).cpu().numpy())

            # Advance hidden state
            h_prev = cell(x_t, h_prev)

    tau_matrix = np.array(taus).T   # (hidden_size, T)
    ...
    im = ax1.imshow(tau_matrix, aspect='auto', cmap='viridis',
                    origin='lower', interpolation='nearest')
    ax1.set_xlabel('Time Step')
    ax1.set_ylabel(f'Neuron index (layer {layer_idx})')
    ax1.set_title('τ(x, h)  per neuron and time step')
    fig.colorbar(im, ax=ax1, label='τ value')

    mean_tau = tau_matrix.mean(axis=0)
    ax2.plot(mean_tau, color='darkviolet', linewidth=1.5, label='Mean τ')
    ax2.fill_between(range(len(mean_tau)),
                     tau_matrix.min(axis=0),
                     tau_matrix.max(axis=0),
                     alpha=0.3, color='mediumorchid', label='[min, max] τ')
    ax2.axhline(cell.tau_min, color='red', linestyle='--',
                linewidth=1, label=f'τ_min = {cell.tau_min}')
    ax2.set_xlabel('Time Step')
    ax2.set_ylabel('τ')
    ax2.set_title('Mean time constant across neurons')
...
def train_model(
    ...
    **loss_kwargs,
    ...
    """
    Generic training loop for ``LNNModel``.

    Args:
        model       : ``LNNModel`` instance.
